...
from destructible import DestructibleComponent
from attacker import AttackComponent
...
class StartScreen(Screen):
    ...
    def draw(self, con):
        lbt.console_print_ex(con,
                             lbt.console_get_width(con) / 2,
                             lbt.console_get_height(con) / 2,
                             lbt.BKGND_NONE, lbt.CENTER, 
...
class PlayScreen(Screen):
    ...
    def __init__(self, width, height, game):
        ...
        self.world = worldgen.create_cavernous_world(width * 2,
                                                     height * 2, 
                                                     1)
        self.world.screen = self
        self.w_console = lbt.console_new(self.screen_width,
                                         self.screen_height)
        self.game = game
        self.add_entities()
        self.add_items()
    ...
    def add_entities(self):
        ...
        monster_stats = Stats("stats", 50, 50, 3, 5, 5, 5)
        player_x, player_y = self.world.empty_location(0)
        player = Entity(player_x,
                        player_y, 0,
                        '@', "player")
        ...
        player.add_component(AttackComponent("attack", 15))
        player.add_component(DestructibleComponent("destructible",
                                                   8))
        player.add_component(Inventory("inventory", 10))
        ...
        for x in xrange(0, monster_count):
            ...
            m.add_component(copy.deepcopy(monster_stats))
            m.add_component(AttackComponent("attack", 10))
            m.add_component(DestructibleComponent("destructible",
                                                  10))
            self.world.add_entity(m)

    def add_items(self):
        for _ in xrange(200):
            _color = lbt.Color(random.randint(0,255),
                              random.randint(0,255),
                              random.randint(0,255))
            gem = Item("gem", '*', color=_color, uses=1)
            self.world.add_item_random_location(gem, 0)

    def get_scroll_x(self):
        return max(0, min(self.world.player.x - self.screen_width / 2,
                          self.world.width - self.screen_width))

    def get_scroll_y(self):
        return max(0, min(self.world.player.y - self.screen_height / 2,
                          self.world.height - self.screen_height))
            ...
            for y in xrange(self.screen_height):
                ...
                if fov.can_see(wx, wy, p.z):
                    lbt.console_put_char_ex(self.w_console,
                                            x, y,
                                            self.world.char_at(wx,
                                                               wy,
                                                               p.z),
                                            self.world.color_at(wx,
                                                                wy,
                                                                p.z),
                                            lbt.black)
    ...
    def draw_gui(self, con):
        ...
        gui.print_location_info(con,
                                20, 
                                self.screen_height + 3,
                                self.world.tile_at(plr.x, 
                                                   plr.y,
                                                   plr.z))
